...
fn parse_all_results_page(
    ...
    for (idx, element) in table_body.enumerate() {
        ...
        let handle = std::thread::spawn(move || {
            ...
            let gp_result = fetch_parse_individual_sprint(body)?;
            let mut guraded_data = output_arc_clone
                .lock()
                .map_err(|_| Error::ParseRaceResults)?;
            guraded_data.push(CompletedSprint {
                round: idx + 1,
                gp_name,
                results: gp_result,
            });
...
fn is_sprint_weekend(document: &Html) -> Result<bool> {
    let sidebar_ul_selector =
        scraper::Selector::parse("ul.f1-sidebar-wrapper > li > a").map_err(|_| Error::Scraper)?;

    let sidebar_ul = document.select(&sidebar_ul_selector);

    for (_, element) in sidebar_ul.enumerate() {
        let x = element.text().collect::<Vec<_>>();
        if x.len() == 1 && x[0] == "Sprint" {
            return Ok(true);
        }
    }
    return Ok(false);
...
fn fetch_parse_individual_sprint(body: String) -> Result<Vec<PositionInfo>> {
    let td_selector = scraper::Selector::parse("td").map_err(|_| Error::Scraper)?;
    ...
    let y = is_sprint_weekend(&document)?;
    println!("IS SPRINT WEEKEND {y}");
    if !y {
        return Ok(vec![]);
    }
...
pub struct CompletedSprint {
    round: usize,
    gp_name: String,
    results: Vec<PositionInfo>,
}

impl DataFetcher for CompletedSprint {
    type A = Vec<CompletedSprint>;

    fn cache_file_name() -> String {
        format!("{}_sprint_results.json", *CURR_YEAR)
    }
    ...
    fn process_data(raw_data: String, file_path: &Path) -> Result<Self::A> {
        let mut all_results: Vec<CompletedSprint> = if file_path.exists() {
...
// impl CompletedSprint {
//     pub fn pp_completed_sprint_results(&self, output: &mut String) -> Result<()> {
